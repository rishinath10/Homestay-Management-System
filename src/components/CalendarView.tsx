...
export const CalendarView: React.FC<CalendarViewProps> = ({
  ...
  // Selected date state (defaults to null; only highlighted when user clicks a date)
  ...
  const renderHeader = () => {
    const activeDateStr = selectedDateStr || format(currentDate, 'yyyy-MM-dd');
    const activeDateObj = parseISO(activeDateStr);
    
    let titleStr = format(currentDate, 'MMMM yyyy');
    if (viewMode === 'day') {
      titleStr = format(activeDateObj, 'EEEE, MMMM d, yyyy');
    } else if (viewMode === 'week') {
      const weekStart = startOfWeek(currentDate);
      const weekEnd = endOfWeek(currentDate);
      titleStr = `${format(weekStart, 'MMM d')} - ${format(weekEnd, 'MMM d, yyyy')}`;
    } else if (viewMode === 'property_grid') {
      titleStr = `Timeline Matrix (${format(currentDate, 'MMM d')} - ${format(addDays(currentDate, 13), 'MMM d, yyyy')})`;
    } else if (viewMode === 'agenda') {
      titleStr = 'Upcoming Bookings Agenda';
    }
    ...
    return (
      <div className="hidden md:flex items-center justify-between py-3 px-4 bg-white border-b border-gray-250 shadow-2xs select-none">
        <div className="flex items-center space-x-3 shrink min-w-0">
          ...
          <div className="flex items-center space-x-1 shrink-0">
            <button
              onClick={clickToday}
              className="px-3 py-1 border border-gray-300 hover:bg-gray-50 text-gray-700 text-xs font-semibold rounded-full transition-colors focus:outline-hidden"
            >
              Today
            </button>
            <button
              onClick={clickPrev}
              className="p-1.5 text-gray-650 hover:bg-gray-105 rounded-full transition-colors focus:outline-hidden"
              title="Previous"
            >
        ...
        <div className="flex items-center space-x-3 shrink-0">
          {onViewModeChange && (
            <div className="bg-gray-100 p-0.5 rounded-lg flex items-center text-xs font-medium text-gray-700">
              <button
                onClick={() => onViewModeChange('month')}
                className={`px-3 py-1 rounded-md transition-all ${
                  viewMode === 'month' ? 'bg-white text-blue-600 shadow-xs font-semibold' : 'hover:text-gray-950'
                }`}
              >
                Month
              </button>
              <button
                ...
                className={`px-3 py-1 rounded-md transition-all ${
                  viewMode === 'day' ? 'bg-white text-blue-600 shadow-xs font-semibold' : 'hover:text-gray-950'
                }`}
              >
                Day
              </button>
              <button
                ...
                className={`px-3 py-1 rounded-md transition-all ${
                  viewMode === 'agenda' ? 'bg-white text-blue-600 shadow-xs font-semibold' : 'hover:text-gray-950'
                }`}
              >
                List
              </button>
          ...
          {activeRole !== 'staff' && (
            <button
              onClick={() => onCreateBookingAtDate(viewMode === 'day' ? selectedDateStr : format(currentDate, 'yyyy-MM-dd'))}
      ...
      return (
        ...
        >
          {mWeeks.map((week, weekIdx) => {
            ...
            return (
              <div key={weekIdx} className="relative bg-gray-200 h-full overflow-hidden min-h-0">
                    ...
                    return (
                      <div
                        ...
                        onClick={() => {
                          ...
                          if (!isPastDay && activeRole !== 'staff') {
                            onCreateBookingAtDate(dateStr);
                          }
                        }}
                        className={`p-1 flex flex-col justify-between transition-all group h-full ${
                          isPastDay
                      ...
                      >
                        ...
                        {/* No mobile dots, labels are rendered in the absolute layer below */}
                      </div>
                    );
                  })}
                </div>
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center space-x-0.5 md:space-x-1 truncate leading-none">
                          {!isStartInWeek && (
                            <span className="text-[7px] md:text-[9px] opacity-80 font-mono">‹</span>
                          )}
                        ...
                        <div className="flex items-center space-x-0.5 shrink-0 ml-0.5 md:ml-1 leading-none">
                          {isMultiDay && isStartInWeek && (
                            <span className="text-[6px] md:text-[7.5px] bg-black/25 px-0.5 md:px-1 rounded font-mono font-bold shrink-0">
                              {nights}N
                            </span>
                          )}
                          {!isEndInWeek && (
                            <span className="text-[7px] md:text-[9px] opacity-85 font-mono">›</span>
                          )}
                        </div>
                      </div>
                    );
            ...
            <div ref={containerRef} className="overflow-hidden flex-1 relative min-h-0 w-full">
              <div
                ...
                style={{
                  ...
                  transition: useTransition ? 'transform 450ms cubic-bezier(0.19, 1, 0.22, 1)' : 'none',
        ...
        {selectedDateStr && (
          <div className="bg-white border-t border-gray-200 px-3 sm:px-6 py-2.5 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2 shadow-lg z-20">
            <div className="flex items-center justify-between sm:justify-start space-x-3">
              ...
              <span className="px-2.5 py-0.5 bg-yellow-100 text-yellow-900 border border-yellow-300 text-[11px] font-bold rounded-full">
  ...
  if (viewMode === 'day') {
    ...
    return (
      <div className="flex-1 bg-gray-50 flex flex-col h-full overflow-hidden">
        {renderHeader()}
        <div className="flex-1 p-4 md:p-6 lg:p-8 flex flex-col overflow-y-auto space-y-6">
          {/* Date Selector Header */}
          <div className="bg-white rounded-2xl p-5 border border-gray-200 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-4 md:hidden">
          ...
          <div className="flex items-center space-x-3">
            ...
            {activeRole !== 'staff' && (
              <button
                onClick={() => onCreateBookingAtDate(activeDateStr)}
        ...
        <div className="space-y-4">
          <h3 className="text-xs font-bold text-gray-500 uppercase tracking-wider">
            All 5 Port Dickson Villas Status for {activeDateStr}
          </h3>
                    ...
                    <div className="p-4 flex-1">
                      {isBooked && booking ? (
                        ...
                        <div className="h-full flex flex-col items-center justify-center p-6 text-center text-gray-400 border border-dashed border-gray-200 rounded-xl space-y-2">
                          ...
                          {activeRole !== 'staff' && (selectedDateStr >= format(new Date(), 'yyyy-MM-dd')) ? (
                            <button
                            ...
                            selectedDateStr < format(new Date(), 'yyyy-MM-dd') && (
                              <span className="text-[11px] text-gray-400 font-semibold italic">Past date — booking disabled</span>
                            )
                          )}
                        </div>
                      )}
        ...
        <div className="flex-1 p-4 md:p-6 lg:p-8 flex flex-col overflow-y-auto">
          <div className="max-w-6xl mx-auto w-full space-y-6">
            <div className="bg-white p-6 rounded-3xl border border-gray-200 shadow-sm space-y-4">
              <div className="border-b border-gray-200 pb-3 flex items-center justify-between md:hidden">
                ...
                >
                  ...
                  <div className="space-y-1.5">
                    {dayBookings.map((b) => {
                      ...
                      const color = prop?.color || '#1a73e8';
                      const todayStr = format(new Date(), 'yyyy-MM-dd');
                      const isPastBooking = (b.endDate || b.bookingDate) < todayStr;
      ...
      <div className="flex-1 bg-gray-50 flex flex-col h-full overflow-hidden">
        ...
        <div className="flex-1 p-4 md:p-6 lg:p-8 flex flex-col overflow-hidden">
          <div className="bg-white rounded-3xl border border-gray-200 shadow-sm flex-1 flex flex-col overflow-hidden">
            <div className="p-4 bg-gray-50/70 border-b border-gray-200 flex items-center justify-between text-xs text-gray-700 md:hidden">
