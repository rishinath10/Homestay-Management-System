  ...
  return (
    <header className="h-16 bg-white border-b border-gray-200 px-3 sm:px-4 flex items-center justify-between sticky top-0 z-30 shadow-xs select-none">
        ...
        <div className="hidden md:flex items-center space-x-2 mr-1 sm:mr-3 shrink-0">
          ...
          <div>
            <div className="flex items-center space-x-1.5">
              <span className="font-semibold text-gray-900 text-sm sm:text-base leading-none">PDHV Management System</span>
            </div>
      ...
      <div className="flex items-center space-x-1.5 sm:space-x-3 shrink-0">
        ...
        <button
          onClick={async () => {
            const token = await requestNotificationPermission(userEmail || undefined);
            if (token || (typeof window !== 'undefined' && 'Notification' in window && Notification.permission === 'granted')) {
              alert('🔔 Push notifications are active! Assigned staff will receive alerts when new bookings are created.');
            } else if (typeof window !== 'undefined' && 'Notification' in window && Notification.permission === 'denied') {
              alert('⚠️ Notification permission is blocked in browser settings. Please click the site settings icon in your browser URL bar to allow notifications.');
            } else {
              alert('Push notification permission requested.');
            }
          }}
          className="p-1.5 text-gray-600 hover:bg-gray-100 rounded-full transition-colors relative shrink-0"
          title="Enable Push Alerts for Assigned Bookings"
        >
        ...
        {/* Clear Cache & Hard Sync Button */}
        <button
          onClick={async () => {
            try {
              localStorage.removeItem('pd_bookings_cache');
              localStorage.removeItem('pd_properties_cache');
              if ('caches' in window) {
          ...
          className="hidden md:flex items-center space-x-1.5 px-3 py-1.5 bg-blue-50 hover:bg-blue-100 text-blue-700 rounded-lg text-xs font-bold transition-colors border border-blue-200 shrink-0"
          title="Clear Browser Cache & Sync"
        >
        ...
        <div className="relative shrink-0">
          <button
            ...
            className="flex items-center justify-center w-10 h-10 hover:bg-gray-100 rounded-full transition-colors border border-gray-200"
          >
            {activeStaff?.avatarUrl ? (
              <img src={activeStaff.avatarUrl} alt={userName || 'User'} className="w-8 h-8 rounded-full object-cover" />
            ) : (
          ...
          {showUserMenu && (
            <div className="absolute right-0 mt-2 w-56 bg-white rounded-xl shadow-lg border border-gray-200 py-2 z-50">
              <div className="px-4 py-3 border-b border-gray-100">
                ...
                <p className="text-[10px] mt-1.5 px-2 py-0.5 bg-gray-100 text-gray-600 font-bold uppercase rounded w-fit">
              ...
              <button
                ...
                className="w-full text-left px-4 py-2.5 text-xs font-bold text-blue-700 hover:bg-blue-50 flex items-center space-x-2.5 transition-colors border-b border-gray-100"
              >
              ...
              <button
                ...
                className="w-full text-left px-4 py-3 text-xs font-bold text-red-650 hover:bg-red-50 flex items-center space-x-2.5 transition-colors"
              >
                <LogOut className="w-4.5 h-4.5 text-red-550" />
                <span>Sign Out</span>
              </button>
            </div>
          )}
